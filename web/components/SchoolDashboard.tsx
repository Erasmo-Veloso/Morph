...
export function SchoolDashboard() {
  ...
  return <main className="school-shell">
    ...
    <section className="school-grid school-grid-bubble">
      <article className="school-card school-bubble-card school-bubble-editor">
        ...
        <div className="school-bubble-controls">
          ...
          <label>Raio pedagógico <span>{bubble.boundary.radius_meters} m</span><input aria-label="Raio pedagógico" type="range" min="5" max="1000" step="5" value={bubble.boundary.radius_meters} onChange={(event) => setBubble((current) => ({ ...current, boundary: { ...current.boundary, radius_meters: Number(event.target.value) } }))} /></label>
        ...
        <div className="bubble-editor-footer"><p aria-live="polite"><MapPin size={15} /> {bubble.boundary.radius_meters < 50 ? "Espaço pequeno: a validação exige precisão GPS adequada." : message}</p><div className="bubble-editor-actions"><Link className="school-secondary-link" href="/teacher">Abrir Studio do professor <ArrowRight size={14} /></Link><button type="button" onClick={saveBubble} disabled={isSaving}><Save size={16} /> {isSaving ? "A guardar…" : "Guardar School Bubble"}</button></div></div>
      </article>
    ...
    <section className="school-card school-people-card"><div className="school-card-header"><div><p>COMUNIDADE AUTORIZADA</p><h2>Professor, turma e alunos da demonstração</h2></div><span className="school-tag">DEMO SEM LOGIN</span></div><div className="teacher-list">{(school?.teachers ?? []).map((teacher, index) => <div className={teacher.id === selectedTeacher?.id ? "teacher-row selected-teacher" : "teacher-row"} key={teacher.id}><div className="teacher-avatar">{String(index + 1).padStart(2, "0")}</div><div><strong>{teacher.name}</strong><small>{teacher.subject} · {teacher.className}</small></div><span>{teacher.studentIds.length} alunos</span><CheckCircle2 size={18} /></div>)}</div>{selectedTeacher ? <div className="school-assignment"><Users size={17} /><span><strong>{selectedTeacher.name}</strong> pode iniciar Capsules para {selectedTeacher.className}; o runtime do <strong>Aluno de demonstração</strong> reconhece essa relação para a demonstração.</span><Link href="/teacher">Abrir aula <ArrowRight size={15} /></Link></div> : null}<div className="school-assignment"><PackagePlus size={17} /><span><strong>Dispositivo principal</strong> · {runtime?.enrollment?.state === "PAIRED" ? `${runtime.enrollment.deviceName ?? "Android Morph"} associado a ${runtime.enrollment.studentName}.` : `Introduza o código ${runtime?.enrollment?.pairingCode ?? "MORPH-2026"} no Android para associar o Aluno de demonstração.`} <small>Uma associação é suficiente para esta demo.</small></span><span className="school-tag">{runtime?.enrollment?.state ?? "PENDING"}</span></div></section>
