...
"$ADB" shell pm list packages | grep -q '^package:com.morph.runtime$' && echo "MORPH_INSTALLED: yes" || echo "MORPH_INSTALLED: no"
ACCESSIBILITY_SERVICE="com.morph.runtime/com.morph.runtime.PolicyAccessibilityService"
if "$ADB" shell settings get secure enabled_accessibility_services | tr ':' '\n' | grep -qx "$ACCESSIBILITY_SERVICE"; then
